...
| `gather-auth.js` | Gather's own auth: adopts the desktop client's Firebase session out of IndexedDB, refreshes ID tokens, and does authenticated REST calls. `refreshSessionForPairing` re-adopts at every `pair` so the phone follows whichever account the Mac last used; `signOutGather` is the reset. |
