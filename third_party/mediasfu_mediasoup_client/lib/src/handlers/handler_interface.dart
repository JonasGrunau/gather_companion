...
abstract class HandlerInterface extends EnhancedEventEmitter {
  ...
  /// PATCHED (gather-v2-app, 2026-09-17): when `run()` has actually finished.
  ///
  /// `run()` is declared `void ... async`, so the `Transport` constructor starts
  /// it and returns immediately while the RTCPeerConnection is still being
  /// built. Anything that produces before that lands on `_pc!` and throws a null
  /// check — which `FlexQueue` swallows, so the caller just waits out its own
  /// timeout. Awaiting this closes the race.
