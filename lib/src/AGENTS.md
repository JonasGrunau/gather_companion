...
| `bridge_client.dart` | `BridgeClient` — one WebSocket with sequence-based catch-up and ping-driven liveness. Also `LinkState` / `LinkStatus`, `recentHistory()` for a first connection, and `whenLive()` for pull-to-refresh. |
